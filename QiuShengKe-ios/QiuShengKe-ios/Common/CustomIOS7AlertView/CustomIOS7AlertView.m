...
- (void)close
{
    ...
    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionTransitionNone
					 animations:^{
                         ...
                         if ([self viewWithTag:[@"bannerView" hash]]) {
                             [self viewWithTag:[@"bannerView" hash]].layer.transform = CATransform3DConcat(currentTransform, CATransform3DMakeScale(0.6f, 0.6f, 1.0));
                         }
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if ([buttonTitles[i] isEqualToString:@"分享"]) {
            [closeButton setTitleColor:COLOR(82, 183, 243, 1) forState:UIControlStateNormal];
            [closeButton setImage:[UIImage imageNamed:@"bth_share"] forState:UIControlStateNormal];
        ...
        if (i != 0) {
            ...
            line.frame = CGRectMake(i * buttonWidth, container.bounds.size.height - buttonHeight, 1, buttonHeight);
...
#pragma mark - 自定义
//customed by sxc
- (void)showTopBannerView:(UIView *)bannerView {
    if (!bannerView) {
        return;
    }
    if ([self viewWithTag:[@"bannerView" hash]]) {
        [[self viewWithTag:[@"bannerView" hash]] removeFromSuperview];
    }
    bannerView.frame = CGRectMake((CGRectGetWidth(self.frame)-CGRectGetWidth(bannerView.frame))/2, CGRectGetMinY(self.dialogView.frame)-CGRectGetHeight(bannerView.frame), bannerView.frame.size.width, bannerView.frame.size.height);
    bannerView.tag = [@"bannerView" hash];
    bannerView.layer.transform = CATransform3DMakeScale(0, 0, 0);
    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionCurveEaseInOut
                     animations:^{
                         bannerView.layer.opacity = 1.0f;
                         bannerView.layer.transform = CATransform3DMakeScale(1, 1, 1);
                     }
                     completion:NULL
     ];
