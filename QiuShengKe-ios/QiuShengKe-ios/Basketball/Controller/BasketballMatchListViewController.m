...
@property(nonatomic, strong)NSDictionary* nbaF;
...
@implementation BasketballMatchListViewController
...
- (void)loadData{
    ...
    [[QiuMiHttpClient instance]GET:[NSString stringWithFormat:QSK_MATCH_BASKET_LIST,_selectDate] parameters:nil cachePolicy:QiuMiHttpClientCachePolicyHttpCache prompt:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        self.currentF = [[responseObject objectForKey:@"filter"] objectAtIndex:0];
        self.allF = [[responseObject objectForKey:@"filter"] objectAtIndex:0];
        self.nbaF = [[responseObject objectForKey:@"filter"] objectAtIndex:3];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    BasketballMatchTableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:@"BasketballMatchTableViewCell"];
    if(cell == nil){
        cell = [[NSBundle mainBundle] loadNibNamed:@"BasketballMatchTableViewCell" owner:nil
                                           options:nil][0];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    BasketballMatchDetailViewController* controller = (BasketballMatchDetailViewController*)[QiuMiCommonViewController controllerWithStoryBoardName:@"Basketball" withControllerName:@"BasketballMatchDetailViewController"];
    if([_showMatches count] > indexPath.row){
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return [BasketballMatchTableViewCell heightOfCell];
}
...
- (IBAction)clickFilter:(UIButton*)sender{
    ...
    else if([sender.titleLabel.text isEqualToString:@"NBA"]){
        [_filterBtn2 setTitleColor:QIUMI_COLOR_C1 forState:UIControlStateNormal];
        _currentF = _nbaF;
        for (NSDictionary* data in [_nbaF objectForKey:@"data"]) {
            [self.firstLids addObject:[data objectForKey:@"id"]];
        }
    }
